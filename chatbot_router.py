...
async def chat_with_uptime_bot(req: ChatRequest, db: Session = Depends(get_db)):
    ...
    if any(x in user_msg for x in ["حالة النظام", "status", "system status", "الوضع العام"]):
        up_count = sum(1 for m in monitors if not m.checks or m.checks[-1].is_up)
        down_count = len(monitors) - up_count
        return {"reply": f"📊 تقرير حالة النظام:\n• عدد المواقع المراقبة: {len(monitors)}\n• تعمل بنجاح: {up_count} ✅\n• متوقفة: {down_count} ❌"}

    # 2. Down Sites (المواقع المتوقفة)
    if any(x in user_msg for x in ["المواقع المتوقفة", "errors", "down sites", "المشاكل", "issues"]):
        down_sites = []
        for m in monitors:
            last = db.query(CheckResult).filter(CheckResult.monitor_id == m.id).order_by(CheckResult.checked_at.desc()).first()
            if last and not last.is_up:
                down_sites.append(f"• {m.name}: {last.error}")
        
        if down_sites:
            return {"reply": "❌ المواقع التي تواجه مشاكل حالياً:\n" + "\n".join(down_sites)}
        return {"reply": "✅ ممتاز! لا توجد أي مواقع متوقفة حالياً. جميع الأنظمة تعمل بكفاءة."}

    # 3. Slowest Sites (أبطأ المواقع)
    if any(x in user_msg for x in ["أبطأ المواقع", "slow", "performance", "الأداء", "speed"]):
        ...
        if not scored:
            return {"reply": "⚠️ لم يتم جمع بيانات الأداء بعد. يرجى الانتظار قليلاً أو تشغيل فحص جديد."}
    ...
    if any(x in user_msg for x in ["آخر فحص", "latest", "recent", "فحص"]):
        return {"reply": "🔍 يمكنك الضغط على زر 'Check Now' في لوحة التحكم لتشغيل فحص فوري لأي موقع. سيظهر لك التقرير فوراً في القائمة."}

    # 5. Help / Greeting
    return {"reply": "مرحباً بك! 👋 أنا مساعدك الآلي. يمكنك سؤالي عن:\n1️⃣ حالة النظام\n2️⃣ المواقع المتوقفة\n3️⃣ أبطأ المواقع\n\nأو اضغط على الأزرار المقترحة أعلاه!"}
...
def generate_smart_fallback(msg: str, monitors: List[Monitor], db: Session) -> str:
    ...
    if target_site:
        last = db.query(CheckResult).filter(CheckResult.monitor_id == target_site.id).order_by(CheckResult.checked_at.desc()).first()
        is_up = not last or last.is_up
        status_text = "يعمل بشكل جيد ✅" if is_up else f"متوقف حالياً ❌ (السبب: {last.error if last else 'غير معروف'})"
        ...
        if target_site.perf_score:
            ...
            reply += f"• سرعة التحميل (FCP): {target_site.perf_fcp or 'غير متوفر'} ثانية\n"
    ...
    if any(x in msg for x in ["حالة", "الوضع", "status", "health"]):
        ...
        return f"جميع المواقع الـ {len(monitors)} التي أراقبها تعمل بشكل ممتاز حالياً! ✅"
